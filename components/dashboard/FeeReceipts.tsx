...
/**
 * Printable receipt cards for every fee a student has paid — the admission
 * application fee (when `admission.paid`) and each paid course-fee checkout.
 * Shared by the student's own `/fees/receipt` page and the staff student
 * detail view. Renders nothing when there is nothing paid.
 */
export function FeeReceipts({ admission, fees }: { admission: Admission | null; fees: FeeStructureRow[] }) {
  ...
  if (!admission?.paid && paidFees.length === 0) return null;
